...
export const titleUpdateParser = (
  data: ILinearUpdateData<IIssueData>
): string => {
...
export const stateUpdateParser = (
  data: ILinearUpdateData<IIssueData>
): string => {
...
export const labelUpdateParser = (
  ...
  const removedLabelId = oldLabelIds.find(
    (labelId: string) => !newLabelIds.includes(labelId)
  );
  if (addedLabelId) {
...
export const assigneeUpdateParser = (
  data: ILinearUpdateData<IIssueData>
): string => {
  const { data: issue, url } = data;
...
export const updateIssueParser = (data: ILinearData<IIssueData>) => {
  ...
  switch (updatedField?.key) {
    case 'title':
      return titleUpdateParser(data);
    case 'stateId':
      return stateUpdateParser(data);
    case 'assigneeId':
